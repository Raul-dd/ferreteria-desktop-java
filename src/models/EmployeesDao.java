...
public class EmployeesDao {
    ...
    public boolean updateEmployeeQuery(Employees employee) {
        String query = "UPDATE employees "
                + "SET full_name = ?, username = ?, address = ?, telephone = ?, email = ?, rol = ?, updated = ? "
                + "WHERE id = ?";

        Timestamp datemite = new Timestamp(new Date().getTime());
        ...
        try {
            ...
            pst.setTimestamp(7, datemite);
            pst.setInt(8, employee.getId());
            pst.execute();
            return true;
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Error al modificar los datos del empelado" + e);
            return false;
        }
    }
